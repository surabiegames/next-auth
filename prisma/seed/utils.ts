...
import { GolonganTarif, KategoriPembacaan, KondisiCatat, StatusPelanggan, UkuranMeter } from "../../src/generated/prisma";
 ...
 */
export function normalizePencatat(raw: string | undefined | null): string | null {
  if (!raw) return null;
...
export function normalizeNotelp(raw: string | undefined | null): string | null {
  if (!raw) return null;
  const s = raw.trim();
  if (s === "" || s === "0" || s === "00000" || s === "-" || s === "000") return null;
  return s;
}
...
export async function inBatches<T>(
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    process.stdout.write(`  batch ${batchIndex}/${totalBatches} (${i + batch.length}/${total})\r`);
