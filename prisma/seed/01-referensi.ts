...
import { GolonganTarif } from "../../src/generated/prisma";
...
export async function seedReferensi() {
  ...
  for (const d of wilDistData) {
    const wilAdm = await prisma.wilayahAdm.findUnique({ where: { kode: d.wilayahAdmKode } });
    if (!wilAdm) continue;
  ...
  for (const d of seksiData) {
    const wilDist = await prisma.wilayahDist.findUnique({ where: { kode: d.wilDistKode } });
    if (!wilDist) continue;
  ...
  for (const d of zonaData) {
    const wilSeksi = await prisma.wilayahSeksi.findUnique({ where: { kode: d.wilSeksiKode } });
  ...
  log("01-REFERENSI", `Zona: ${zonaData.length} records (sisanya di-seed di fase 02)`);
  ...
  const kecamatanData = [
    ...
    { kode: "TC", nama: "BOJONGLOA KALER" },
    { kode: "CC", nama: "CIBEUNYING KIDUL" }, 
  ];
  ...
  const tarifData: Array<{
    ...
    { kode: GolonganTarif.GOL_1A,  kodeAsli: "1A",  nama: "SOS.UMUM/CORSEN,R.IBADAH", kategori: "Sosial" },
    { kode: GolonganTarif.GOL_1B,  kodeAsli: "1B",  nama: "SOSIAL KHUSUS",             kategori: "Sosial" },
    { kode: GolonganTarif.GOL_2A1, kodeAsli: "2A1", nama: "RMH.TANGGA GOL.A1",         kategori: "Rumah Tangga" },
    { kode: GolonganTarif.GOL_2A2, kodeAsli: "2A2", nama: "RMH.TANGGA GOL.A2",         kategori: "Rumah Tangga" },
    { kode: GolonganTarif.GOL_2A3, kodeAsli: "2A3", nama: "RMH.TANGGA GOL.A3",         kategori: "Rumah Tangga" },
    { kode: GolonganTarif.GOL_2A4, kodeAsli: "2A4", nama: "RMH.TANGGA GOL.A4",         kategori: "Rumah Tangga" },
    { kode: GolonganTarif.GOL_2A5, kodeAsli: "2A5", nama: "RMH.TANGGA GOL.A5",         kategori: "Rumah Tangga" },
    { kode: GolonganTarif.GOL_2B,  kodeAsli: "2B",  nama: "INSTANSI PEMERINTAH",        kategori: "Pemerintah" },
    { kode: GolonganTarif.GOL_3A,  kodeAsli: "3A",  nama: "NIAGA KECIL",                kategori: "Niaga" },
    { kode: GolonganTarif.GOL_3B,  kodeAsli: "3B",  nama: "NIAGA MENENGAH/BESAR",       kategori: "Niaga" },
    { kode: GolonganTarif.GOL_3C,  kodeAsli: "3C",  nama: "NIAGA",                      kategori: "Niaga" },
    { kode: GolonganTarif.GOL_4A,  kodeAsli: "4A",  nama: "INDUSTRI KECIL",             kategori: "Industri" },
    { kode: GolonganTarif.GOL_4B,  kodeAsli: "4B",  nama: "IND.MNGH/BESAR/PERK.",       kategori: "Industri" },
  ];
  ...
  const konfig = [
    { kunci: "TARIF_BEBAN",            nilai: "7000",  tipe: "number", deskripsi: "Biaya beban/abonemen tetap per bulan (Rp)" },
    { kunci: "TARIF_ADMIN",            nilai: "10000", tipe: "number", deskripsi: "Biaya administrasi per bulan (Rp)" },
    { kunci: "TARIF_AIR_KOTOR",        nilai: "11100", tipe: "number", deskripsi: "Biaya air limbah per bulan (Rp)" },
    { kunci: "HARI_JATUH_TEMPO",       nilai: "20",    tipe: "number", deskripsi: "Hari setelah tanggal faktur untuk jatuh tempo" },
    { kunci: "DENDA_PERSEN",           nilai: "2",     tipe: "number", deskripsi: "Persentase denda keterlambatan per bulan (%)" },
    { kunci: "AMBANG_ANOMALI_PERSEN",  nilai: "50",    tipe: "number", deskripsi: "Threshold penyimpangan pemakaian untuk flagging anomali (%)" },
    { kunci: "BATCH_SIZE_IMPORT",      nilai: "500",   tipe: "number", deskripsi: "Jumlah baris per batch saat import CSV" },
  ];
