...
import { JenisPemutusan, StatusPelanggan } from "../../src/generated/prisma";
...
export async function seedRNomor(csvPath: string) {
  ...
  for (const row of rows) {
    ...
    if (!nolg || !jenis || isNaN(periode)) { stats.skip++; continue; }
    if (jenis !== "TSM" && jenis !== "SPT") { stats.skip++; continue; }
    ...
    if (pelanggan) {
      ...
      log("05-RNOMOR", `ℹ Pelanggan ${nolg} tidak ditemukan (sudah dicabut sebelum closing) — tetap insert Pemutusan`);
      stats.noMatch++;
    }
  ...
  log("05-RNOMOR", `✓ TSM: ${stats.tsm}, SPT: ${stats.spt}, Tidak ditemukan: ${stats.noMatch}, Skip: ${stats.skip}`);
}