...
import { JenisMutasi } from "../../src/generated/prisma";
import {
  readCsv, log,
  normalizeNolg, normalizeNotelp, normalizeMerk,
  mapTarif, mapUkuranMeter, excelDateToJs,
} from "./utils";

/**
 * Decode kode_wilayah: "KC30401" → { ruteKode: "KC304", noUrut: 1 }
 */
function decodeKodeWilayah(raw: string): { ruteKode: string; noUrut: number } | null {
...
export async function seedPBPK(csvPath: string) {
  ...
  for (const row of rows) {
    ...
    if (ruteKode) {
      if (!cacheRute.has(ruteKode)) {
        const seksi = await prisma.seksiCater.findFirst({ where: { kode: "C5" } });
        if (seksi) {
    ...
    const geoLong = row["geo_long"] && parseFloat(row["geo_long"]) < 200
      ? parseFloat(row["geo_long"]) : null;
    const geoLat  = row["goe_lat"] && parseFloat(row["goe_lat"]) < 200
      ? parseFloat(row["goe_lat"]) : null;
    ...
    if (jenis === "PB") {
      ...
      const pelanggan = await prisma.pelanggan.upsert({
        ...
        update: {
          ...
          rt, rw, notelp, geoLat, geoLong,
          jumlahPenghuni:  row["jmlpenghuni"] ? parseInt(row["jmlpenghuni"]) : null,
      ...
      if (!pelanggan) {
        ...
        log("04-PBPK", `ℹ PK: pelanggan ${nolg} (${row["nama"]?.trim()}) tidak ada di DB → buat baru`);
        ...
        await prisma.pelanggan.update({
          ...
          data: {
            ...
            jumlahPenghuni:  row["jmlpenghuni"] ? parseInt(row["jmlpenghuni"]) : undefined,
          },
        });
        ...
        if (nometer) {
          ...
          if (aktif && aktif.nomorMeter !== nometer) {
            await prisma.meter.update({ where: { id: aktif.id }, data: { isAktif: false } });
            await prisma.meter.create({
              data: {
                nomorMeter:    nometer,
                merkKode:      normalizeMerk(row["kd_merkmeter"]),
  ...
  log("04-PBPK", `✓ PB: ${stats.pb}, PK update: ${stats.pk}, PK baru: ${stats.pkBaru}, Skip: ${stats.skip}`);
}