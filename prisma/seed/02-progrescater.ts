...
import {
  ...
  normalizeNolg, normalizePencatat, normalizeMerk, normalizeNotelp,
  mapTarif, mapStatusPelanggan, mapKondisi, mapUkuranMeter, mapKategori,
  parseDate, thblToDate,
} from "./utils";
...
export async function seedProgresCater(csvPath: string) {
  ...
  allWilSeksi.forEach((r: (typeof allWilSeksi)[0])     => cacheWilSeksi.set(r.kode, r.id));
  allSeksiCater.forEach((r: (typeof allSeksiCater)[0]) => cacheSeksiCater.set(r.kode, r.id));
  ...
  allTarif.forEach((r: (typeof allTarif)[0])           => cacheTarif.set(r.kodeAsli, r.id));
  allPencatat.forEach((r: (typeof allPencatat)[0])     => cachePencatat.set(r.namaLapangan, r.id));
  ...
  await inBatches(rows, 500, async (batch: Record<string, string>[]) => {
    for (const row of batch) {
      ...
      if (zonaKode && !cacheZona.has(zonaKode)) {
        const wilSeksiId = cacheWilSeksi.get(row["wilseksikode"]?.trim() ?? "") ?? null;
        if (wilSeksiId) {
      ...
      if (ruteKode && !cacheRute.has(ruteKode)) {
        const seksiId = cacheSeksiCater.get(row["caterseksikode"]?.trim() ?? "") ?? null;
        if (seksiId) {
      ...
      const seksiCaterId = cacheSeksiCater.get(row["caterseksikode"]?.trim() ?? "") ?? null;
      ...
      const pelanggan = await prisma.pelanggan.upsert({
        ...
        update: {
          ...
          seksiCaterId, ruteId, zonaId, kecamatanId: kecId, kelurahanId: kelId,
        },
        create: {
          nomorLangganan:  nolg,
      ...
      if (!nometer || nometer === "") { stats.skip++; continue; }
      ...
      if (existingMeter) {
        if (existingMeter.nomorMeter !== nometer) {
          ...
          const meter = await prisma.meter.create({
            data: {
              nomorMeter:    nometer,
              nomorSegel:    row["nosegelmeter"] ? String(row["nosegelmeter"]).trim() : null,
              merkKode:      normalizeMerk(row["kd_merkmeter"]),
              ukuran:        mapUkuranMeter(row["ukmeter"]),
              tanggalPasang: parseDate(row["tglpasangmeter"]),
              umurTahun:     row["umurmeterthn"] ? parseInt(row["umurmeterthn"]) : null,
              umurBulan:     row["umurmeterbln"] ? parseInt(row["umurmeterbln"]) : null,
              isAktif:       true,
      ...
      const pembacaan = await prisma.pembacaanMeter.upsert({
        ...
        update: {
          ...
          blokTarifLalu: row["blok_m3lalu"] ? parseInt(row["blok_m3lalu"]) : null,
      ...
      const tagnunggak = row["tagnunggak"] && row["tagnunggak"] !== "" && !isNaN(Number(row["tagnunggak"]))
        ? BigInt(Math.round(parseFloat(row["tagnunggak"])))
        : null;
      const jmlRekNunggak = row["jmlreknunggak"] && row["jmlreknunggak"] !== ""
      ...
      await prisma.tagihan.upsert({
        ...
        update: {
          jmlHargaAir, beaBeban, beaAdmin, airKotor, lainLain, totalTagihan,
  ...
  log("02-PROGRESCATER", `✓ Pelanggan: ${stats.pelanggan}, Meter: ${stats.meter}, Pembacaan: ${stats.pembacaan}, Tagihan: ${stats.tagihan}, Skip: ${stats.skip}`);
}